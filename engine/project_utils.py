...
def carregar_estrutura_projeto():
    ...
    for caminho in sorted(raiz.rglob("*")):
        ...
        if caminho.is_dir():
            resultado.append(f"[DIR] {relativo}")
        elif caminho.suffix == ".md":
            resultado.append(f"[FILE] {relativo}")
    return "\n".join(resultado)
...
# ==============================================================================
# NOVAS FUNÇÕES EXTRAÍDAS DO FRONT-END (LÓGICA DE NEGÓCIOS E ARQUIVOS)
# ==============================================================================

def encontrar_arquivo_por_wikilink(nome_alvo: str) -> Path | None:
    """Busca no projeto o arquivo .md correspondente ao nome de um Wikilink."""
    alvo_norm = normalizar_nome(nome_alvo)
    raiz = Path(CAMINHO_PROJETO)
    
    candidatos = []
    for arq in raiz.rglob("*.md"):
        if any(part in IGNORELIST for part in arq.parts):
            continue
        if normalizar_nome(arq.stem) == alvo_norm:
...
def criar_documento_wikilink(nome_alvo: str, pasta_destino: str, nome_origem: str) -> Path:
    """Cria um novo documento Markdown derivado de um clique em Wikilink inexistente."""
...
def filtrar_caminhos_busca(query: str) -> set[str]:
    """Retorna os caminhos de arquivos e pastas pais que correspondem a uma pesquisa."""
...
def calcular_estatisticas_texto(texto: str) -> tuple[int, int, int, int]:
    """Retorna (palavras, caracteres, linhas, tokens_estimados) para o editor."""
    if not texto:
        return 0, 0, 0, 0
...
def otimizar_memoria_ram():
    """Força o SO a esvaziar a memória de trabalho inativa no Windows."""
...
def criar_novo_arquivo(parent_dir: str, nome: str, template_escolhido: str, obter_template_func) -> tuple[bool, str, Path | None]:
    """Cria um arquivo .md no diretório pai aplicando o template selecionado."""
    if not nome.lower().endswith(".md"):
        nome += ".md"

    caminho_arquivo = os.path.join(parent_dir, nome)
    if os.path.exists(caminho_arquivo):
        return False, "Um arquivo com esse nome já existe.", None
...
def renomear_item_projeto(caminho: str, novo_nome: str) -> tuple[bool, str, Path | None]:
    ...
    if novo_nome == nome_antigo:
        return False, "O novo nome é idêntico ao anterior.", None

    novo_caminho = os.path.join(diretorio_pai, novo_nome)
    if os.path.exists(novo_caminho):
        return False, "Já existe um arquivo ou pasta com esse nome.", None

    try:
        os.rename(caminho, novo_caminho)
        return True, f"Renomeado: '{nome_antigo}' -> '{novo_nome}'", Path(novo_caminho)
    except Exception as e:
        return False, f"Falha ao renomear: {e}", None

def deletar_item_projeto(caminho: str) -> tuple[bool, str]:
    """Remove um arquivo ou pasta do sistema de arquivos."""
    ...
    try:
        ...
        return False, f"Falha ao deletar item: {e}"

def duplicar_item_projeto(caminho: str) -> tuple[bool, str, Path | None]:
    """Duplica um arquivo ou pasta gerando um sufixo numérico de cópia."""
    try:
        ...
        return False, f"Falha ao duplicar item: {e}", None

def mover_item_projeto(src_path: str, target_path: str) -> tuple[bool, str, Path | None]:
    """Move um arquivo ou pasta prevenindo aninhamento circular."""
    src_abs = os.path.abspath(src_path)
    target_abs = os.path.abspath(target_path)

    dest_dir = target_abs if os.path.isdir(target_abs) else os.path.dirname(target_abs)
    if os.path.isdir(src_abs) and dest_dir.startswith(src_abs):
        return False, "Não é possível mover uma pasta para dentro dela mesma.", None

    dest_path = os.path.join(dest_dir, os.path.basename(src_abs))
    if os.path.abspath(src_abs) == os.path.abspath(dest_path):
        return False, "Origem e destino são o mesmo caminho.", None

    try:
        shutil.move(src_abs, dest_path)
        return True, f"Movido '{os.path.basename(src_abs)}' para '{os.path.basename(dest_dir)}'", Path(dest_path)
    except Exception as e:
        return False, f"Falha ao mover item: {e}", None

def colar_item_clipboard(src_path: str, target_path: str, mode: str) -> tuple[bool, str, Path | None]:
    """Executa a operação de Colar (Copiar ou Recortar) da área de transferência."""
    if not src_path or not os.path.exists(src_path):
        return False, "Nenhum item válido na área de transferência.", None
    ...
    try:
        ...
        return False, f"Falha ao colar item: {e}", None

def abrir_no_explorador_nativo(caminho: str):
    """Abre o arquivo ou pasta no gerenciador de arquivos padrão do SO."""
...
def formatar_historico_chat(memorias: str) -> list[tuple[str, str]]:
    """Converte a string bruta de memórias salva em uma lista estruturada de diálogos."""
    ...
    for i in range(1, len(blocos), 2):
        ...
        if header == "Prompt Usuário:":
            resultado.append(("You", content))
        elif header == "Resposta:":
            resultado.append(("Ao", content))
        elif header == "Resumo de Memórias:":
            resultado.append(("System", f"[Resumo de Diálogos Anteriores]: {content}"))

    return resultado

def preparar_prompt_conversa_ao(prompt: str, user_name: str, memorias: str, arquivo_anexo: dict = None) -> tuple[str, str]:
    """Monta as instruções do sistema e o payload do usuário para envio à IA."""
    persona = (
    ...
    conteudo_prompt = ""
    if arquivo_anexo:
        conteudo_prompt += f"--- ARQUIVO ANEXADO PELO USUÁRIO ({arquivo_anexo['name']}) ---\n{arquivo_anexo['content']}\n\n"
    if memorias:
...
def obter_prompts_auditoria_lore() -> tuple[str, str]:
    """Retorna a instrução do sistema e o prompt estruturado para a auditoria de lore."""
    system_instruction = (
...
# 🛡️ Relatório de Auditoria de Lore - {PASTA_PROJETO}
