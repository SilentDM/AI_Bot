...
class SilentDesktopApp:
    ...
    def toast(self, mensagem, duration=3500):
        """Exibe uma notificação flutuante e elegante no canto inferior direito."""
        def _create_toast():
            try:
    ...
    # ------------------------------------------------------------------
    # CONTROLE DE CHAT E PENSAMENTO DE AO
    # ------------------------------------------------------------------
    ...
    def show_thinking(self):
        """Exibe o indicador dinâmico de pensamento do Ao no Chat."""
        self.chat_display.config(state=tk.NORMAL)
    ...
    def remove_thinking_and_respond(self, text):
        """Remove a linha de 'Thinking' e insere a resposta final de Ao."""
        self.chat_display.config(state=tk.NORMAL)
        try:
